import { validationRules, ValidationSchema } from '../composables/useFormValidation';
  ...
  name: {
    ...
    rules: [
      ...
      (value: string) => {
        ...
        return !invalidChars.test(value) ? true : 'Numbers and special characters not allowed';
      }
    ]
  },
  email: {
...
export const loginModalSchema: ValidationSchema = {
  url: {
    ...
    rules: [
      ...
      validationRules.maxLength(2048, 'URL is too long (maximum 2048 characters)')
    ]
  },
  username: {
    required: true,
    rules: [
      ...
      validationRules.maxLength(100, 'Username is too long (maximum 100 characters)'),
      (value: string) => {
        if (!value) return true;
        if (value.includes('@')) {
          return validationRules.email('Please enter a valid email address')(value);
        }
        return true;
      }
    ]
  ...
  password: {
    ...
    rules: [
      ...
      validationRules.maxLength(512, 'Password is too long (maximum 512 characters)')
    ]
  },
  notes: {
    required: false,
    rules: [
      validationRules.maxLength(1000, 'Notes are too long (maximum 1000 characters)')
    ]
  }
};
  ...
  title: {
    ...
    rules: [
      ...
      (value: string) => {
        ...
        return !invalidChars.test(value) ? true : 'Title contains invalid characters';
      }
    ]
  },
  content: {
    ...
    rules: [
      ...
      validationRules.maxLength(10000, 'Content is too long (maximum 10,000 characters)')
    ]
  }
};
