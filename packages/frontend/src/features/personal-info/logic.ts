...
export const updatePersonalInfo = async (
  ...
  const phoneNumbersWithIds =
    updates.phoneNumbers?.map((phone) => ({
      ...phone,
      id: crypto.randomUUID(),
    })) || [];

  const emailsWithIds =
    updates.emails?.map((email) => ({
      ...email,
      id: crypto.randomUUID(),
    })) || [];
...
export const getPrimaryContact = (personalInfo: PersonalInfo) => {
  const primaryPhone =
    personalInfo.phoneNumbers.find((p) => p.isPrimary) ||
    personalInfo.phoneNumbers[0];
  const primaryEmail =
    personalInfo.emails.find((e) => e.isPrimary) || personalInfo.emails[0];

  return {
    phone: primaryPhone
      ? formatPhoneNumber(primaryPhone.number, primaryPhone.countryCode)
      : null,
